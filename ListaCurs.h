...
#include "Processo.h" /* InicializarProcesso(), ImprimirProcesso(), typedef Processo*/
#include <math.h> /* abs() */
...
int _ulttopri(TListaCurs *lc,int velho_ultimo_de_valor, int novo_ultimo); //Percorre os processo de uma lista de cursores do ulitmo item ao primeiro
int _pritoult(TListaCurs *lc,int velho_ultimo_de_valor, int novo_ultimo); //Percorre os processo de uma lista de cursores do primeiro item ao ultimo
int TLCInsere(TListaCurs *lc);				//Insere item na lista em ordem crescente(chamando as funçoes _ulttopri _pritoult)
